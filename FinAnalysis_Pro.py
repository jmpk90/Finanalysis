...
# ─────────────────────────────────────────────
# THEME PALETTES — 3 distinctly different looks
# ─────────────────────────────────────────────
THEMES = {
    # ── THEME 1: Clean Light ──────────────────────────────────
    "clean": {
        ...
        "--glow-green":    "rgba(21,128,61,0.08)",
        "--glow-red":      "rgba(220,38,38,0.08)",
        "--glow-gold":     "rgba(37,99,235,0.08)",
        "--navbar-bg":     "#0f172a",
        "--navbar-text":   "#f8fafc",
        "--score-bg":      "#f1f5f9",
        "--is-light":      "1",
    ...
    "dusk": {
        ...
        "--glow-red":      "rgba(248,113,113,0.10)",
    ...
    "rose": {
        ...
        "--accent-purple": "#7c3aed",
        "--glow-blue":     "rgba(37,99,235,0.08)",
        "--glow-green":    "rgba(45,106,79,0.08)",
        "--glow-red":      "rgba(185,28,28,0.08)",
        "--glow-gold":     "rgba(190,90,56,0.10)",
        "--navbar-bg":     "#2d1f1a",
        "--navbar-text":   "#faf7f4",
        "--score-bg":      "#f5f0eb",
        "--is-light":      "1",
...
def inject_theme():
    ...
    theme_key = st.session_state.get("theme", "clean")
    t = THEMES.get(theme_key, THEMES["clean"])
...
# ─────────────────────────────────────────────
# EQUITEX LUXURY DARK THEME
# Obsidian base · Platinum text · Electric gold
# Fonts: Cormorant Garamond + DM Mono + Outfit
# ─────────────────────────────────────────────
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700&family=Fira+Code:wght@300;400;500;600&display=swap');
...
def render_navbar():
    ...
    col_nav, col_actions = st.columns([5, 3])
    with col_nav:
        page = st.radio(
            "nav",
    ...
    with col_actions:
        ta, tb = st.columns([2, 2])
        with ta:
            theme_options = {"clean": "☀ Clean Light", "dusk": "🌇 Warm Dusk", "rose": "🌸 Rose Sand"}
            current = st.session_state.get("theme", "clean")
            chosen = st.selectbox(
                "Theme", list(theme_options.values()),
                index=list(theme_options.keys()).index(current),
                key="theme_select", label_visibility="collapsed"
            )
            new_key = [k for k,v in theme_options.items() if v == chosen][0]
            if new_key != current:
                st.session_state.theme = new_key
                st.rerun()
        with tb:
            if st.button("＋ Add Portfolio", key="nav_add_pf", use_container_width=True):
    ...
    with st.sidebar:
        ...
        st.markdown('<div style="border-top:1px solid var(--border);margin:16px 0 12px;"></div>',
                    unsafe_allow_html=True)
        if st.button("＋ Add Portfolio", key="sb_add_pf", use_container_width=True):
            st.session_state.show_add_portfolio = True
            st.rerun()
...
def render_add_portfolio_modal():
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:6px;padding:20px 24px;margin-bottom:20px;">
        <div style="font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:600;color:var(--accent-gold);margin-bottom:4px;">ADD NEW PORTFOLIO</div>
...
def page_overview():
    ...
    with col_t:
        ...
        <div style="padding:0 0 16px;">
            <div style="font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:600;color:var(--text-primary);">PORTFOLIO OVERVIEW</div>
    ...
    if not portfolios:
        ...
        <div style="text-align:center;padding:80px 20px;">
            <div style="font-family:'Cormorant Garamond',serif;font-size:48px;font-weight:600;color:var(--text-muted);margin-bottom:12px;">NO PORTFOLIOS</div>
            <div style="font-family:var(--font-mono,'Fira Code',monospace);font-size:11px;color:var(--text-muted);">Click ＋ Add Portfolio to get started</div>
        </div>""", unsafe_allow_html=True)
        return
    ...
    avoid_count    = sum(1 for r in all_results if r.get("decision")=="AVOID")
    ...
    tab_labels = ["📊 OVERVIEW"] + [f"💼 {pf['name'].upper()}" for pf in portfolios]
                ...
                with col:
                    ...
                    st.markdown(f"""<div class="cmp-card">
                        <div class="cmp-hdr">
                            ...
                            <div style="margin-left:auto;font-family:var(--font-mono,'Fira Code',monospace);font-size:9px;color:var(--text-muted);">{len(pf.get('stocks',[]))} STOCKS</div>
                        </div>
                        <div style="padding:12px 16px;">
                        ...
                        <div style="display:flex;gap:4px;padding:8px 12px;border-top:1px solid #1e2d3d;">
                            <div style="flex:1;text-align:center;background:rgba(0,200,122,0.08);border:1px solid rgba(0,200,122,0.2);border-radius:2px;padding:3px;font-family:var(--font-mono,'Fira Code',monospace);font-size:10px;color:#00c87a;">B {pf_buy}</div>
                            <div style="flex:1;text-align:center;background:rgba(255,176,0,0.08);border:1px solid rgba(255,176,0,0.2);border-radius:2px;padding:3px;font-family:var(--font-mono,'Fira Code',monospace);font-size:10px;color:#ffb000;">H {pf_hold}</div>
                            <div style="flex:1;text-align:center;background:rgba(255,59,92,0.08);border:1px solid rgba(255,59,92,0.2);border-radius:2px;padding:3px;font-family:var(--font-mono,'Fira Code',monospace);font-size:10px;color:#ff3b5c;">A {pf_avoid}</div>
                        </div>
                    </div>""", unsafe_allow_html=True)

        if not all_results:
            st.markdown('<div class="info-block">💡 Select a portfolio tab and click <b>Analyze Portfolio</b> to see scores and signals.</div>', unsafe_allow_html=True)
        ...
        if all_results:
            ...
            for pf in portfolios:
                ...
                for r in pf.get("results",[]):
                    pnl_c = "#00c87a" if r.get("pnl_pct",0)>=0 else "#ff3b5c"
                    ...
                    rows_html += f"""<tr>
                        <td><div style="display:flex;align-items:center;gap:6px;">
                            ...
                            <span style="font-family:var(--font-mono,'Fira Code',monospace);font-size:9px;color:var(--text-muted);">{pf['name'][:10]}</span>
                        </div></td>
                        ...
                        <td class="mono" style="color:{'#00c87a' if (r.get('pe') or 99)<25 else '#ffb000'};">{f"{r.get('pe'):.1f}x" if r.get('pe') else '—'}</td>
                        <td class="mono">{dy}</td>
                        <td>{div_badge}</td>
    ...
    for i,(pf,tab) in enumerate(zip(portfolios, tabs[1:])):
        with tab:
...
def page_analyzer():
    st.markdown("""
    <div style="padding:0 0 16px;">
        <div style="font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:600;color:var(--text-primary);">STOCK DEEP DIVE</div>
        <div style="font-family:var(--font-mono,'Fira Code',monospace);font-size:10px;color:var(--text-muted);margin-top:2px;">
    ...
    if R is None:
        st.markdown("""
        <div style="text-align:center;padding:80px 20px;">
            <div style="font-family:'Cormorant Garamond',serif;font-size:48px;font-weight:600;letter-spacing:4px;color:#1e2d46;margin-bottom:12px;">ENTER A TICKER</div>
        ...
        for i,(name,sc,mx,clr,pos,neg) in enumerate(sources):
            with col_list[i%3]:
                ...
                st.markdown(f"""<div style="background:linear-gradient(145deg,#111827,#0d1220);border:1px solid #1e2d46;
                  ...
                  <div style="font-family:'Cormorant Garamond',serif;font-size:32px;font-weight:700;color:{clr};line-height:1;">{sc:.0f}<span style="font-size:12px;color:#2e4060;">/{mx}</span></div>
...
def page_compare():
    st.markdown("""
    <div style="padding:0 0 16px;">
        <div style="font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:600;color:var(--text-primary);">PORTFOLIO COMPARISON</div>
...
def page_dashboard():
    ...
    cards = [
        ("Stock P&L", f"{fmt_inr(total_pnl)}", f"{pnl_pct:+.1f}% · {total_stocks} stocks",
         "var(--accent-green)" if total_pnl>=0 else "var(--accent-red)"),
        ...
        ("Loans Outstanding",  fmt_inr(loan_total),  "Total liability", "var(--accent-red)"),
    ]
    for col,(lbl,val,sub,ac) in zip([c1,c2,c3,c4], cards):
        with col:
            st.markdown(f"""<div style="background:var(--bg-card);border:1px solid var(--border);
    ...
    if portfolios:
        st.markdown(f'<div style="font-size:13px;font-weight:600;color:var(--text-primary);margin-bottom:10px;">Stock Portfolios</div>', unsafe_allow_html=True)
        pf_cols = st.columns(min(len(portfolios), 4))
        for i, (pf, col) in enumerate(zip(portfolios, pf_cols)):
            results   = pf.get("results", [])
            pf_inv    = sum(r.get("invested",0) for r in results) or sum(s.get("invested",0) for s in pf.get("stocks",[]))
            pf_mkt    = sum(r.get("market_value",0) for r in results) or pf_inv
            pf_pnl    = pf_mkt - pf_inv
            pf_pct    = (pf_pnl/pf_inv*100) if pf_inv>0 else 0
            color     = pf.get("color", portfolio_color(i))
            pnl_c     = "var(--accent-green)" if pf_pnl>=0 else "var(--accent-red)"
            with col:
                st.markdown(f"""<div style="background:var(--bg-card);border:1px solid var(--border);
                    border-radius:8px;padding:14px 16px;">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                        <div style="width:8px;height:8px;border-radius:50%;background:{color};"></div>
                        <div style="font-size:13px;font-weight:600;color:var(--text-primary);">{pf['name']}</div>
                    </div>
                    <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px);">
                        {len(pf.get('stocks',[]))} stocks · Invested {fmt_inr(pf_inv)}</div>
                    <div style="font-size:16px;font-weight:600;color:{pnl_c};">{pf_pct:+.1f}%</div>
                </div>""", unsafe_allow_html=True)
    else:
