...
        elif price > sma200:
            score += 0.5; reasons.append(" Price slightly above 200 DMA — weak uptrend")
        else:
...
        elif rsi < 35:
            score += 0.5; reasons.append(f" RSI oversold at {rsi:.0f} — potential bounce")
        elif rsi > 75:
...
            score += 0.5; reasons.append(f" Moderate risk-reward: {upside*100:.1f}% upside vs {downside*100:.1f}% downside")
        else:
...
        elif pe < 40:  score += 0.5; reasons.append(f" PE {pe:.1f}x — moderately expensive (25–40x)")
...
        elif roe>0.12: score += 0.5; reasons.append(f" ROE {pct(roe)} — adequate (12–20%)")
...
        elif de < 100: score += 0.5; reasons.append(f" D/E {de:.1f} — moderate leverage")
...
        elif rg > 0.05:score += 0.5; reasons.append(f" Revenue growth {pct(rg)} — moderate")
...
        elif margin > 0.05:score += 0.3; reasons.append(f" Net margin {pct(margin)} — thin margins")
...
                st.error(f" Maximum {MAX_PORTFOLIOS} portfolios reached. Delete one first.")
                return
...
        st.markdown(f'<div class="warn-block"> Delete portfolio <b>{pf.get("name","")}</b>? This cannot be undone.</div>', unsafe_allow_html=True)
...
        if alerts:
            for a in alerts:
                st.markdown(f'<div class="warn-block"> {a["msg"]}</div>', unsafe_allow_html=True)
...
        if df_raw.empty:
            st.markdown(f'<div class="warn-block"> No data for "{st.session_state.az_ticker}". Try with .NS suffix (e.g. RELIANCE.NS)</div>', unsafe_allow_html=True)
            return
...
        err = R.get("error","")
        st.markdown(f'<div class="warn-block"> No data available for <b>{st.session_state.az_ticker}</b>{(": " + err) if err else ""}. Click <b>Analyze</b> to fetch fresh data, or check the ticker symbol.</div>', unsafe_allow_html=True)
...
            st.markdown('<div class="warn-block"> No price data available for this ticker.</div>', unsafe_allow_html=True)
        else:
            # ── Chart controls ────────────────────────────────
...
                </div>""", unsafe_allow_html=True)
            st.markdown(f'<div class="warn-block"> STOP LOSS: ₹{bp["stop_loss"]} (2× ATR of ₹{bp["atr"]})</div>', unsafe_allow_html=True)
        with bp_c2:
...
    sub = st.tabs([" Stocks", " Mutual Funds", " Analyzer", " Compare"])
